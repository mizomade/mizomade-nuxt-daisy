<template>
  <div style="margin-bottom:90rem;max-width:100%;" class=" flex flex-col justify-center  items-start mt-10 mb-44 lg:w-4/5 lg:mx-auto lg:max-w-10/12 ">
    <div class="container mx-2"  style="width:96%">
      <div class="flex lg:flex-row flex-col md:flex-wrap">
        ...
        <div class="mb-6 flex flex-col lg:ml-10">
          <label
            class="mr-4  text-gray-700 font-bold inline-block mb-2"
            for="name"
            >Category</label
          ...
          >
            <option  v-for="cat in categories_list" :key="cat.name" :value="cat.id" class="rounded">{{cat.name}}</option>
...
setup(){
  ...
  methods: {
    ...
    titlevalidation(){
      if(this.title != null && this.title.length <! 5 && this.title.length >! 220 ){
  ...
  mounted(){
    ...
    //   console.log(error);
    // });
    this.categories_list = this.currentstore.categoriesList;
